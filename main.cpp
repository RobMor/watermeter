...
place places[MAX_PLACES],   // array of places, each of which is a box containing digits (class place is defined in digits.h)
    *dragging = NULL;       // pointer to the place currently being created or resized
int numplaces = 0;          // current number of places
...
int main(int argc, char *argv[]) {
    ...
    // load all of the #.ppm images for comparison
    digit::init(); //REMEMBER the cmpimages won't update when saved until the program's restarted
    ...
    makewindow();
    makepipeline(window, &mainimage);
    
    //g_idle_add(timeout, NULL);
    
    capture(&mainpixbuf, &mainimage);
    gtk_widget_set_size_request(drawingarea, mainimage.width, mainimage.height);
    
    // load place coords
    //FILE *placef = fopen("places.csv", "r");
    //if (placef) {
        //for (numplaces = 0; numplaces < MAX_PLACES; numplaces++) {
            //if (places[numplaces].scanbounds(placef)) break;
            //places[numplaces].update(&mainimage);
        //}
        //fclose(placef);
    //}
    redraw();
    
    // enter the main loop, which will call our callback functions for keyboard/mouse/display/etc events
    mainloop = g_main_loop_new(NULL, FALSE);
    g_main_loop_run(mainloop);
    
    // save place coords
    //placef = fopen("places.csv", "w");
    //for (int i = 0; i < numplaces; i++)
        //places[i].printbounds(placef);
    //fclose(placef);
    
    // clean up and leave
    cleanupwindow();
    cleanuppipeline(mainimage.data, mainpixbuf);
    digit::cleanup();
...
void redraw() {
    ...
    for (int i = 0; i < numplaces; i++) {
        strcat(buf, places[i].getvaluestr());
        strcat(buf, ",");
    }
...
gboolean keypress(GtkWidget *widget, GdkEventKey *event, gpointer data) {
    ...
    if (running) {
        ...
        if (event->keyval >= 0x30 && event->keyval <= 0x39) {
            
            int num = event->keyval - 0x30;
            
            // if the key is CTRL + 0-9, save the most recent place to a file
            if (numplaces && event->state&GDK_CONTROL_MASK) {
                char buf[12];
                sprintf(buf, "%d.ppm", num);
                places[numplaces-1].saveto(buf);
                return TRUE;
            } else if (num > 0 && num < 4) {
                needleMethod = num;
                recalc();
            ...
            switch (event->button) {
                ...
                case 3:
                    if (!running) {
                        ...
                        // try to start dragging existing place boxes
                        // (startdrag returns true if x,y is close enough to a corner or edge)
                        int i;
                        for (i = 0; i < numplaces; i++)
                            if (places[i].startdrag(event->x, event->y)) {
                                dragging = &places[i];
                                break;
                            }
                        
                        // if none of them accepted the drag, create a new place box
                        if (i == numplaces) {
                            numplaces++;
                            if (numplaces > MAX_PLACES) numplaces = MAX_PLACES;
                            dragging = &places[numplaces-1];
                            dragging->forcestartdrag(event->x, event->y);
                        }
            ...
            switch (event->button) {
                ...
                case 3:
                    if (dragging) {
                        ...
                        // stop dragging
                        dragging->enddrag(mainimage.width, mainimage.height);
                        
                        // tell it to update its thresholdimage, digit locations, and guessed value
                        dragging->update(&mainimage);
                        
                        // no longer dragging
                        dragging = NULL;
                        
                        // now that the place has changed, redraw ourself (and histogram)
                        redraw();
                        if (hist->window) gdk_window_invalidate_rect(hist->window, NULL, FALSE);
                    }
                    break;
...
gboolean motion(GtkWidget *widget, GdkEventMotion *event, gpointer data) {
    if (!running) {
        ...
        // right button is pressed; continue dragging place box
        if (event->state & GDK_BUTTON3_MASK) {
            if (dragging) {
                dragging->drag(event->x, event->y, mainimage.width, mainimage.height);
                redraw();
            }
            return TRUE;
        }
...
gboolean expose(GtkWidget *widget, GdkEventExpose *event, gpointer data) {
    ...
    gdk_draw_pixbuf(event->window, NULL, mainpixbuf, 0, 0, 0, 0,
        mainimage.width, mainimage.height, GDK_RGB_DITHER_NONE, 0, 0);

    // call each place's drawing rountine for red and green boxes, and its threshold image
    for (int i = 0; i < numplaces; i++)
        places[i].draw(event->window, red, green);
